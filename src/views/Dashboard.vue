      ...
      <CCol :md="12">
        <CCard class="mb-4">
          <CCardBody>
            <CRow>
              <CCol :sm="10">
                <CButton  @click="$router.push('/tests/nouveloffre')" class="offre" v-if="user.role=='recruteur' || user.role=='admin'"><i class="fa fa-plus">Offre</i> </CButton>
                <div class="topnav">
                  <div class="search-container">
                    <form>
                      <input class="marginI" type="text" placeholder="entrer votre ville" name="search" v-model="filterparadd">
                      <p class="ih"><i class="fa fa-map-marker"></i></p>
                    </form>
                  </div>
                  <form>
                    <input class="marginII" type="text" placeholder="mots clés ou entreprise" name="search"  v-model="filterpartout">
              ...
              <i class="fa fa-close" v-if="user.id==offre.user_id || user.role=='admin'" @click="deleteOffre(offre.id)" style="margin-left:370px;"></i>
              <i class="fa fa-pencil" v-if="user.id==offre.user_id" @click="sendOffre(offre.id)" style="margin-left:370px;margin-top:10px;"></i>
                      <div class="card-body">
                        <h6><b>{{offre.name}}</b></h6> <h6 @click="profilsendr(user.id)"> poster par : <b>{{offre.poster}} {{offre.poster2}}</b></h6>
                        <p>{{offre.address}}</p>
                        <p>Tel : {{offre.telefone}}</p>
                        <p>categorie : {{offre.categorie}}</p>
                        Entreprise : <img class="img" :src="offre.logo"> {{offre.company}}
                        <p>{{offre.description}}</p>
                          <CButton v-if="!offre.posted && user.id!=offre.user_id" class="post" @click="postuler(offre.id)">Postuler maintenant</CButton>
                      </div>
                    </div>
                  </div>
                </div>
